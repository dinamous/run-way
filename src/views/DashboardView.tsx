  ...
  const filteredTasks = useMemo(() => {
    return tasks
      .filter((task: any) => {
        ...
        if (filterAssignee) {
          const norm = normaliseTask(task);
          const anyStepHas = norm.steps.some((s: any) => s.assignees.includes(filterAssignee));
          const legacyMatch = task.assignee === filterAssignee;
          if (!anyStepHas && !legacyMatch) return false;
        }
        if (filterSteps.length > 0) {
          const norm = normaliseTask(task);
          const activeTypes = norm.steps.filter((s: any) => s.active).map((s: any) => s.type);
      ...
      .map((task: any) => {
        ...
        if (filterSteps.length > 0) {
          steps = steps.filter((s: any) => filterSteps.includes(s.type));
        }
        if (filterAssignee) {
          steps = steps.filter((s: any) => s.assignees.includes(filterAssignee));
        }
        return { ...norm, steps };
      });
