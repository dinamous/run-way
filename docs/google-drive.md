...
1. `useEffect` dispara `googleLoginSilent()` com `prompt: 'none'`
2. Se existir sessão Google ativa no browser, conecta sem interação
3. Se não existir sessão, falha silenciosamente e mostra o botão "Conectar Drive"

**Manual (botão "Conectar Drive"):**
...
5. `load(token)` carrega os dados remotos e substitui o localStorage
