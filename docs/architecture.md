...
│   ├── ViewShell.tsx                  # Wrapper de view: flex col + bg-background; ink strip (.view-ink-strip — sticky, z-40, h-14, fundo azul-tonal com radial-gradient via CSS global) com ViewBreadcrumb; zona de conteúdo (flex-1) abaixo; prop noPadding remove padding; prop subview adiciona crumb extra; prop viewOverride sobrepõe view inferida do LayoutContext
